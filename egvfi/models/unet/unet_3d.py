...
"""
reference:
https://raw.githubusercontent.com/UdonDa/3D-UNet-PyTorch/master/src/model.py
"""
...
def conv_block_3d(in_dim, out_dim, activation, employ_bn):
    kernel_size = (3, 3, 3)
    stride = (1, 1, 1)
    block = nn.Sequential(nn.Conv3d(in_dim, out_dim, kernel_size=kernel_size, stride=stride, padding=1))
    if employ_bn:
        block.append(nn.BatchNorm3d(out_dim))
    block.append(activation)
    return block


def conv_trans_block_3d(in_dim, out_dim, activation, employ_bn):
    kernel_size = (3, 3, 3)
    stride = (2, 2, 2)
    block = nn.Sequential(
        nn.ConvTranspose3d(
...
def conv_block_2_3d(in_dim, out_dim, activation, employ_bn):
    block = nn.Sequential(
        conv_block_3d(in_dim, out_dim, activation, employ_bn),
...
class UNet3D(nn.Module):
    def __init__(self, in_dim, out_dim, num_filters, employ_bn):
        """
        :param in_dim:
        :param out_dim:
        :param num_filters:
        ...
        self.employ_bn = employ_bn
        activation = nn.LeakyReLU(0.2, inplace=True)
        # Down sampling
        self.down_1 = conv_block_2_3d(self.in_dim, self.num_filters, activation, self.employ_bn)
        self.pool_1 = max_pooling_3d()
        self.down_2 = conv_block_2_3d(self.num_filters, self.num_filters * 2, activation, self.employ_bn)
        ...
        # Output
        self.out = conv_block_3d(self.num_filters, out_dim, activation, self.employ_bn)
